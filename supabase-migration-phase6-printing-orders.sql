...
--
-- REVIEW BEFORE EXECUTION IN SUPABASE SQL EDITOR.
...
BEGIN
  ...
  IF jsonb_array_length(v_cart_items) = 0 THEN
    ...
    SELECT name, price
      INTO v_product_name, v_item_price
  ...
  ELSE
    FOR v_index IN 0 .. jsonb_array_length(v_cart_items) - 1 LOOP
      v_item := v_cart_items -> v_index;
      ...
      v_cart_items := jsonb_set(v_cart_items, ARRAY[v_index::TEXT], v_item, true);
      v_subtotal := v_subtotal + (v_item_price * v_item_qty);
    END LOOP;

    v_product_name := CASE
      WHEN jsonb_array_length(v_cart_items) = 1 THEN COALESCE(v_cart_items->0->>'name', 'خدمة طباعة')
      ELSE 'طلب مطبوعات متعدد'
    END;
  END IF;
  ...
  INSERT INTO public.printing_orders (
    ...
    v_order_number, v_product_id, COALESCE(v_product_name, 'خدمة طباعة'), v_customer_name, v_phone, v_notes,
  ...
  RETURN jsonb_build_object(
    ...
    'product_name', COALESCE(v_product_name, 'خدمة طباعة'),
    'subtotal', v_subtotal,
